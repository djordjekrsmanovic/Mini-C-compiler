...
int insert_symbol(char *name, unsigned kind, unsigned type, 
  ...
  symbol_table[index].atr2[0] = atr2;
  return index;
}
...
unsigned get_atr2(int index) {
  if(index > -1 && index < SYMBOL_TABLE_LENGTH)
    return symbol_table[index].atr2[0];
  return NO_ATR;
}
...
void insertFunctionParameters(int index,unsigned atr2,int pozicija){
 if(index > -1 && index < SYMBOL_TABLE_LENGTH){
    symbol_table[index].atr2[pozicija] = atr2;
		//printf("USAO");
	}
}

unsigned get_atr2_parameter(int function_index,int parameter_index){
  if(function_index > -1 && function_index < SYMBOL_TABLE_LENGTH){
		//printf("USAO");
    return symbol_table[function_index].atr2[parameter_index];
	}
  return NO_ATR;
}
